...
LABEL org.opencontainers.image.title="Eliware Oracle Linux 10 GlusterFS"
...
RUN test -n "${RPM_REPO_URL}" \
    ...
    && dnf -y install oracle-epel-release-el10 oraclelinux-developer-release-el10 \
    && dnf config-manager --set-enabled ol10_codeready_builder \
    && dnf -y install gperftools-libs python3-pyxattr \
