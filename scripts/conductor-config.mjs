...
export function createConductorConfig({
  ...
  return {
    repositoryCommitScript: env("CONDUCTOR_REPOSITORY_COMMIT_SCRIPT", ""),
    ...
    // Checkpoints are published beside the artifacts so the Git publication
    // commit captures the exact state used to produce them. Keep the lock and
